...
class Book:
    def __init__(self, id: int, name: str, pages: int):
        if not isinstance(id, int):
            raise TypeError("В поле id должно быть целое число")
        self.id = id

        if not isinstance(name, str):
            raise TypeError("В поле name должна быть строка")
        self.name = name

        if not isinstance(pages, int):
            raise TypeError("В поле pages должно быть целое число")
        self.pages = pages
...
class Library(BaseModel):
    ...
    def get_index_by_book_id(self, book_id: int):
        ...
        if book_index is None:
            raise ValueError("По данному id ничего не найдено")
        return book_index
